...
from sklearn.metrics import mean_squared_error, r2_score, classification_report, \
    confusion_matrix, precision_score, roc_curve, accuracy_score
...
from tet_trading_systems.trading_system_development.data_utils.indicator_feature_workshop.technical_features.standard_indicators \
    import apply_atr, apply_rsi, apply_sma, apply_comparative_relative_strength
from tet_trading_systems.trading_system_development.data_utils.indicator_feature_workshop.technical_features.volume_features \
    import apply_avg_volume, apply_volume_balance
...
from tet_trading_systems.trading_system_development.data_utils.indicator_feature_workshop.technical_features.misc_features \
    import apply_alpha_score, apply_rolling_corr, apply_percent_rank
    ...
    for symbol, df in data_dict.items():
        ...
        X_df.drop(
            [
                ...
                'Volume_benchmark', 'symbol', 'symbol_benchmark',
                'Target',
                'SMA_7', 'SMA_21', 'SMA_49', 'Volume', 'CRS'
            ], 
            axis=1, inplace=True
        )
        # ... or assign columns/features to use as predictors
        ...
        """ 
        SVC
        C = C is 1 by default and it’s a reasonable default choice. 
            If you have a lot of noisy observations you should decrease 
            it: decreasing C corresponds to more regularization. 
        GAMMA = a small gamma will give you low bias and high variance 
            while a large gamma will give you higher bias and low variance. 
                ...
                for i in optimizable_params1:
                    for n in optimizable_params2:
                        ...
                        steps = [
                            ('scaler', StandardScaler()),
                            #('scaler', MinMaxScaler()),
                            ('svm', SVC(C=i, gamma=n, kernel='sigmoid', degree=4, probability=True))
                        ]
                        print(f'*****\nC = {i}\nGAMMA = {n}\n*****')
                        pipeline = Pipeline(steps)
                        pipeline.fit(X_train, y_train)
...
def preprocess_data(
    ...
    for symbol, data in dict(df_dict).items():
        if data.empty or len(data) < target_period:
            ...
            apply_alpha_score(df_dict[symbol], 'benchmark', period_param=9)
            apply_sma(df_dict[symbol], 7, col_name='A_score', suffix='_a_score')
            apply_sma(df_dict[symbol], 21, col_name='A_score', suffix='_a_score')
            apply_sma(df_dict[symbol], 49, col_name='A_score', suffix='_a_score')
            df_dict[symbol]['a_score_ma_diff1'] = df_dict[symbol]['SMA_49_a_score'] - df_dict[symbol]['SMA_21_a_score']
            df_dict[symbol]['a_score_ma_diff2'] = df_dict[symbol]['SMA_21_a_score'] - df_dict[symbol]['SMA_7_a_score']
            apply_atr(df_dict[symbol], period_param=9)
            apply_rsi(df_dict[symbol], period_param=7)
            apply_comparative_relative_strength(df_dict[symbol], 'Close', 'Close_benchmark')
            apply_percent_rank(df_dict[symbol], 63, 'CRS')
            apply_sma(df_dict[symbol], 7)
            apply_sma(df_dict[symbol], 21)
            apply_sma(df_dict[symbol], 49)
            df_dict[symbol]['ma_diff1'] = df_dict[symbol]['SMA_49'] - df_dict[symbol]['SMA_21']
            df_dict[symbol]['ma_diff2'] = df_dict[symbol]['SMA_21'] - df_dict[symbol]['SMA_7']
...
def get_example_ml_system_props(instruments_db: InstrumentsMongoDb, target_period=1):
    system_name = 'stonkinator_flagship'
...
if __name__ == '__main__':
    ...
    model_data_dict = create_svm_models(df_dict, target_period=9)
    ...
    run_trading_system(
        model_data_dict, 'stonkinator_flagship', 
        #ml_entry_regression, ml_exit_regression, 
        ml_entry_classification, ml_exit_classification, 
        #SafeFPositionSizer('sharpe_ratio', 15, 0.85),
        {'req_period_iters': target_period, 'entry_period_lookback': target_period}, 
